...
export class AgencyListComponent implements OnInit {
  ...
  // contains(array, currentName) {
  //   for (let key in array) {
  //     if (array[key].name == currentName)
  //       return true;
  //   }
  //   return false;
  // }
  ...
  getFilteredList() {
    return this.selectedCategories.length === 0 ? this.agencyList : this.agencyList.filter(agency => this.selectedCategories.every(selectedCategory => agency.categories.some(agencyCategory => agencyCategory.id === selectedCategory.id)));
  }
  ...
  toggle(){
    console.log('toggle')
    this.listaZwinieta = !this.listaZwinieta;
    if(this.listaZwinieta){
